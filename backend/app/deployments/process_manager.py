...
def start_process(
    ...
    Sets ``deployment.status = 'starting'``, ``deployment.pid``,
    ``deployment.port`` and ``deployment.url_local``. On failure sets
...
def stop_process(session: Session, deployment: Deployment) -> None:
    """Send SIGTERM to the process; escalate to SIGKILL after 5 s."""
    pid = deployment.pid
    if pid is None:
