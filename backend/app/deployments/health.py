...
def check_health(session: Session, deployment: Deployment) -> bool:
    """Run a single healthcheck and update deployment status. Returns True if healthy."""
...
def _check_static(deployment: Deployment) -> bool:
    ...
    healthcheck = (deployment.healthcheck_path or "/").lstrip("/") or "index.html"
...
def _check_process(deployment: Deployment) -> bool:
    ...
    try:
        with urllib.request.urlopen(url, timeout=_HEALTH_TIMEOUT) as resp:  # noqa: S310
