...
// Task status palette — mirrors TaskList.STATUS_COLOR and adds the
// "cancelled" rendering quirk (strikethrough text) called out in the brief.
...
export function TaskDetail({ taskId }: Props) {
  ...
  if (taskQuery.isError) {
    ...
    return (
      ...
      >
        {notFound ? "El id no corresponde a ninguna tarea." : "No se pudo cargar la tarea."}
      </Alert>
    );
  }
    ...
    <Stack gap="md">
      <Stack gap={4}>
        ...
        <Text c="dimmed" size="xs">
          ...
          {task.completed_at ? ` · Completado ${formatDate(task.completed_at)}` : null}
        </Text>
      </Stack>
